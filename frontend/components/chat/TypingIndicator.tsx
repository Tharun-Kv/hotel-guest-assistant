  ...
  return (
    <div className="flex items-end gap-2.5">
      ...
      <div role="status" aria-live="polite" className="min-w-[230px] rounded-2xl rounded-bl-md border border-violet-100 bg-white px-4 py-3 font-sans text-sm text-slate-600 shadow-sm">
        <div className="flex items-center gap-2 font-semibold text-slate-800"><Timer size={15} className="animate-pulse text-violet-600" aria-hidden="true" /> Working on that <span className="ml-auto text-[10px] font-medium text-slate-400">~2 sec</span></div>
          ...
          {STEPS.map((item, index) => (
            <li key={item.id} className={`flex items-center gap-2 text-xs ${index <= activeIndex ? 'text-slate-700' : 'text-slate-400'}`}>
              <span className="flex h-4 w-4 items-center justify-center">
                {index < activeIndex ? <Check size={13} className="text-emerald-600" aria-hidden="true" /> : index === activeIndex ? <Timer size={13} className="animate-spin text-violet-600" aria-hidden="true" /> : <Circle size={10} className="text-slate-300" aria-hidden="true" />}
              </span>
              {item.label}
            </li>
          ))}
